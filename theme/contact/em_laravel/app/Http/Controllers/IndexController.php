  ...
  public function send( Request $request, $url = null ) {
    ...
    foreach ( $form_items as $key => $row ) {
      ...
      if ( $row['required'] == 1 && $row['name'] ) {
        if ( $row['form_type'] == 'zp_address' ) {
          $vali['zip'.$row['name']]     = "required";
          $vali['pref'.$row['name']]    = "required";
          $vali['address'.$row['name']] = "required";
        }
    ...
    foreach ( $request->all() as $key => $val ) {
      ...
      foreach ( $form_items as $f_val ) {
        ...
        if ( $f_val['form_type'] == 'zp_address' && $key == 'zip'.$f_val['name'] ) {
          $form_req[] = [
            "name"  => 'zp_address',
            ...
            "value" => "\n" . $request->input( 'zip'.$f_val['name'] ) . "\n" . $request->input( 'pref'.$f_val['name'] ) . "\n" . $request->input( 'address'.$f_val['name'] ),
          ];
          continue;
        }
      ...
      foreach ( $form_items as $key => $form_item ) {
        ...
        if ( $form_item['form_type'] == 'zp_address' ) {
          $contents[] = [
            ...
            'value'     => $request->input( 'zip'.$form_item['name'] ) . $request->input( 'pref'.$form_item['name'] ) . $request->input( 'address'.$form_item['name'] )
          ];
        }
