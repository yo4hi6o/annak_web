    ...
    public function update(Request $request)
    {
        ...
        if ( $bytes_written === false ) {
          ...
          if ( !config( 'app.demo_mode' ) ) {
            return redirect()
              ->route( 'admin.theme.show', [ "theme_name" => $request->input( "theme_name" ) ] )
              ->with( [ 'warning' => __( "admin_messages.file_rew_error" ) ] );
          }
          else {
            return redirect()
              ->route( 'admin.theme.show', [ "theme_name" => $request->input( "theme_name" ) ] )
          ...
          if ( !config( 'app.demo_mode' ) ) {
            return redirect()
              ->route( 'admin.theme.show', [ "theme_name" => $request->input( "theme_name" ) ] )
              ->with( [ 'message' => __( "admin_messages.saved" ) ] );
          }
          else {
            return redirect()
              ->route( 'admin.theme.show', [ "theme_name" => $request->input( "theme_name" ) ] )
    ...
    public function get_theme($theme_name = null)
    {
        ...
        foreach ($theme_ar as $key => $path) {
            ...
            if (file_exists($path.'\\screenshot.png') || file_exists($path.'/screenshot.png')) {
                ...
                if (file_exists($path.'\\screenshot.jpg') || file_exists($path.'/screenshot.jpg')) {
