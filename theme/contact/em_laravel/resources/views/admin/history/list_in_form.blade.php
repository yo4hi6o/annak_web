...
@section('content')
    <div class="container-fluid">
        <div class="row mt-3 mb-1">
            <div class="col-3">
                <h3 class="page-header">@isset($form->name)
                        {{ $form->name }}&nbsp;{{__("admin_messages.menu.history")}}
                    @endisset</h3>
            </div>
            <div class="col-9">
                <div class="row">
                    <div class="col-5">
                        ...
                        <a href="{{config("app.url")}}/@isset($form->url){{$form->url}}@endisset"
                           class="btn border-0 btn-outline-dark" target="_blank" data-toggle="tooltip"
                           title="{{ __("admin_messages.form.open_form") }}">
                            {{config("app.url")}}/@isset($form->url)
                                {{$form->url}}
                            @endisset&nbsp;
                        </a>
                    </div>
                    <div class="col-3 text-right">
                        <a href="javascript:void (0);" class="btn btn-outline-dark csv_down_load" data-toggle="tooltip"
                           title="{{ __("admin_messages.csv_download") }}">
                    ...
                    <div class="col-4">
                        <form action="{{route("admin.history.list_in_form")}}" method="post" id="list_in_form"
                              class="float-right" style="max-width: 300px;">
                            @csrf
                            <select id="list_in_form_select" name="form_id" class="form-control"
        ...
        <div class="table-responsive">
            @isset($append['admin_history_upper_table'])
                {!! $append['admin_history_upper_table'] !!}
            @endisset
            <table id="data_table" class="table table-striped table-bordered table-hover font80 m-0" cellspacing="0"
                   width="100%">
                <thead>
                <tr>
                ...
                @foreach($data as $row)
                    <tr>
                        ...
                        @foreach($row->content as $key => $content_row)
                            ...
                            <td>
                                ...
                                @elseif(@is_array($content_row->value))
                                    @foreach($content_row->value as $content_val)
                                        {{$content_val}}&nbsp;
                                    @endforeach
                                @else
                                    {{$content_row->value}}
                        ...
                        <td class="csv_skip">
                            <div class="container-fluid">
                                <form action="{{ route('admin.history.destroy', $row->id) }}" method="post"
                                      id="{{$row->id}}">
                                    @csrf
                                </form>
                                <a href="javascript:void (0);" data-id="{{$row->id}}"
                                   class="btn btn-sm btn-danger listDeleteBtn float-right" data-toggle="tooltip"
                                   title="削除">
            ...
            @isset($append['admin_history_under_table'])
                {!! $append['admin_history_under_table'] !!}
            @endisset
        </div>
    ...
    @isset($append['admin_history_bottom_section'])
        {!! $append['admin_history_bottom_section'] !!}
    @endisset
@endsection
